...
public class Intermediate_lvl_2_A_Adapter extends PagerAdapter {
    Context context;
    PojoIntermediate_level_2_A intermediateLevel2A;
    private boolean isSelected = false;
    public Intermediate_lvl_2_A_Adapter(Context context, PojoIntermediate_level_2_A intermediateLevel2A) {
        this.context = context;
        this.intermediateLevel2A = intermediateLevel2A;
    }

    @Override
    public int getCount() {
        return intermediateLevel2A!= null&&intermediateLevel2A.getData()!=null?intermediateLevel2A.getData().size():0;
    }
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        ...
        View itemView = layoutInflater.inflate(R.layout.intermediate_2_itemview, container, false);

        TextView textViewData = itemView.findViewById(R.id.questionTextView);
        textViewData.setText(intermediateLevel2A.getData().get(position).getQuestion());
        TextView option1 = itemView.findViewById(R.id.option1RadioButton);
        option1.setText(intermediateLevel2A.getData().get(position).getOption1());
        TextView option2 = itemView.findViewById(R.id.option2RadioButton);
        option2.setText(intermediateLevel2A.getData().get(position).getOption2());
        TextView option3 = itemView.findViewById(R.id.option3RadioButton);
        option3.setText(intermediateLevel2A.getData().get(position).getOption3());
        TextView option4 = itemView.findViewById(R.id.option4RadioButton);
        option4.setText(intermediateLevel2A.getData().get(position).getOption4());

        int correctOption = Integer.parseInt(intermediateLevel2A.getData().get(position).getAnswer());
