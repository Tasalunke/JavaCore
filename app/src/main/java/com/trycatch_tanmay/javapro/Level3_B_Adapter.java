...
public class Level3_B_Adapter extends PagerAdapter {
    private Context context;
    private PojoLevel3_A pojoLevel3A;
    private LayoutInflater layoutInflater;
    private boolean isSelected = false;

    public Level3_B_Adapter(Context context, PojoLevel3_A pojoLevel3A) {
        this.context = context;
        this.pojoLevel3A = pojoLevel3A;
        this.layoutInflater = LayoutInflater.from(context);
    }

    @Override
    public int getCount() {
        return pojoLevel3A != null && pojoLevel3A.getData() != null ? pojoLevel3A.getData().size() : 0;
    }
    ...
    public Object instantiateItem(@NonNull ViewGroup container, int position) {
        View itemView = layoutInflater.inflate(R.layout.level_3_itemview, container, false);
        TextView textViewData = itemView.findViewById(R.id.questionTextView);
        textViewData.setText(pojoLevel3A.getData().get(position).getQuestion());
        TextView option1 = itemView.findViewById(R.id.option1RadioButton);
        option1.setText(pojoLevel3A.getData().get(position).getOption1());
        TextView option2 = itemView.findViewById(R.id.option2RadioButton);
        option2.setText(pojoLevel3A.getData().get(position).getOption2());
        TextView option3 = itemView.findViewById(R.id.option3RadioButton);
        option3.setText(pojoLevel3A.getData().get(position).getOption3());
        TextView option4 = itemView.findViewById(R.id.option4RadioButton);
        option4.setText(pojoLevel3A.getData().get(position).getOption4());

        int correctOption = Integer.parseInt(pojoLevel3A.getData().get(position).getAnswer());
